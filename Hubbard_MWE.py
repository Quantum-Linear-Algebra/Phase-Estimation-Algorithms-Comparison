...
if __name__ == "__main__":
    ...
    psi = np.zeros(2**parameters['qubits'])
    valid_states = [i for i in range(2**parameters['qubits']) if bin(i).count("1") == n_target]

    for i in valid_states:
        psi[i] = np.random.randn()
    ...
    for i in range(parameters['qubits']):
        if -n_target//2 < i-parameters['qubits']//2 <= n_target//2:
            print('1')
    ...
    E_approx_list = np.array(E_approx_list)
    error = E_approx_list - target_E

    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))

    axes[0].plot(final_times, E_approx_list, "o-", label=r"$E_{\rm approx}(T)$")
    ...
    axes[1].plot(final_times, np.abs(error), "o-")
